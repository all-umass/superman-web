...
class SpectrumMatchingHandler(MultiDatasetHandler):
    ...
    def post(self):
        ...
        if abs(1 - query[:, 1].max()) > 0.001:
            ...
            logging.warning(
                'Applying max-normalization to query before matching')
        ...
        if self.get_argument('pp', None) is None:
            logging.warning(
                'Applying max-normalization to library before matching')
        ...
        for dv, submask, pkeys in zip(
                ds_views.ds_views, dv_submasks, dv_pkeys):
            names = pkeys[submask]
...
class CompareHandler(BaseHandler):
    ...
    def post(self):
        ...
        if ds.pkey is not None:
            mask = ds.filter_metadata(
                dict(pkey=dict(select=names, search=None)))
        else:
            mask = np.array([int(n.rsplit(' ', 1)[1])
