...
class RegressionModelHandler(GenericModelHandler):
    def get(self, fignum):
        ...
        for ax in fig_data.figure.axes:
            ...
            # there are no actual values to report. Instead, they're random
            # jitter.
            if len(ax.lines) == 6:
                actual.fill(np.nan)
    ...
    def post(self):
        ...
        if do_train is None:
            ...
            if stratify_meta:
                vals, _ = self.collect_one_variable(
                    all_ds_views, stratify_meta)
        ...
        if bool(int(do_train)):
            ...
            if wave.shape != model.wave.shape or not np.allclose(
                    wave, model.wave):
