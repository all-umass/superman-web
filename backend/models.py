...
class GenericModel(object):
    ...
    def load(fh):
        ...
        if cls is None or not issubclass(cls, GenericModel):
            raise ValueError(
                'Invalid model file: class %r not found' % class_name)
    ...
    def save(self, fh):
        ...
        fh.write(b'%s\n%r\n%s\n%r\n%r\n%d\n' % (
            ...
            w.shape[0]))
        # don't use w.tofile(fh) here, because it requires an actual file
        # object
        fh.write(w.tobytes())
...
class KNN(_Classifier):
    ...
    def _prepare_spectra(self, X):
        if len(X) > 0 and np.asanyarray(X[0]).ndim == 2:
            return [np.array(t, copy=False, order='C', dtype=np.float32)
                    for t in X]
...
class PLS1(_UnivariateRegression, _PLS):
    ...
    @classmethod
    def cross_validate(cls, X, variables, comps=None,
                       num_folds=5, labels=None):
        grid = dict(n_components=comps)
        for name, mse_mean, mse_stdv in cls._run_cv(
                X, variables, grid, num_folds, labels=labels):
...
class Lars1(_LassoOrLars1, _Lars):
    @classmethod
    def cross_validate(cls, X, variables, chans=None,
                       num_folds=5, labels=None):
...
class Lasso1(_LassoOrLars1, _Lasso):
    ...
    def cross_validate(cls, X, variables, num_folds=5, labels=None):
        ...
        for key in sorted(variables):
            y, name = variables[key]
            lasso = LassoLarsCV(fit_intercept=False,
                                max_iter=2000, cv=cv, n_jobs=1)
